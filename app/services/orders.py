...
from app.models import Member, MemberTier, Order, OrderStatus, OrderItem, Book
from app.schemas import OrderCreate
from app.services.members import get_member, tier_at_least, RESTRICTED_MIN_TIER
...
def create_order(db: Session, data: OrderCreate, now: datetime) -> Order:
    ...
    for item in data.items:
        ...
        if book.restricted and not tier_at_least(member.tier, RESTRICTED_MIN_TIER):
            raise HTTPException(status_code=403, detail="Member tier too low for restricted book")
